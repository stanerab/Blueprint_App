...
{
    // Escape slashes and replace {param} with regex capture
    $pattern = preg_replace('/\{[a-zA-Z0-9_]+\}/', '([^/]+)', $routePath);
    // Use # delimiter and escape slashes properly
    $pattern = '#^' . preg_quote($pattern, '#') . '$#';
    // Unescape the ([^/]+) parts that were escaped by preg_quote
...
private function handle404()
{
    ...
    $viewPath = APP_PATH . DIRECTORY_SEPARATOR . 'Views' . DIRECTORY_SEPARATOR . 'errors' . DIRECTORY_SEPARATOR . '404.php';
