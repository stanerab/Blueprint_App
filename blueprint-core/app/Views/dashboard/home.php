                        ...
                        <div class="sessions-list">
                            ...
                            <?php foreach ($todaySessions as $session): ?>
                                ...
                                <div class="session-card" 
                                   ...
                                   data-session-room="<?= htmlspecialchars($session->patient_room ?? '') ?>"
    ...
    function closeEditSessionModal() {
        ...
        if (sessionCard) {
                ...
                const sessionData = {
                  ...
                  room_number: sessionCard.dataset.sessionRoom || sessionCard.dataset.currentRoom || '?',
                current_room: sessionCard.dataset.sessionRoom || sessionCard.dataset.currentRoom || '?',
                ...
                fetch('<?= url('sessions/get-by-patient') ?>?id=' + patientId)
                    ...
                    .then(sessions => {
                        ...
                        const sessionData = {
                                ...
                                current_room: session.patient_room || session.room_number || '?',
    ...
    // Update allActivePatients room number
                  ...
                  // Update currentSingleSession room if it belongs to this patient
                    if (currentSingleSession && currentSingleSession.patient_id == patientId && newRoom) {
                        ...
                        currentSingleSession.current_room = newRoom;
                        if (document.getElementById('singleSessionModal').style.display === 'flex') {
                            displaySingleSession(currentSingleSession, currentSingleSession.initials);
                        }
                    }
// Update ALL session cards for this patient in today's session list
                    document.querySelectorAll(`.session-card[data-patient-id="${patientId}"]`).forEach(card => {
                        card.dataset.sessionRoom = newRoom;
                        card.dataset.currentRoom = newRoom;
                        const roomEl = card.querySelector('.session-room');
            ...
            fetch('<?= url('sessions/get-all-json') ?>')
                ...
                .then(data => {
                    ...
                    const mapped = sessions.map(s => ({
                        ...
                        datetime: s.datetime, ward: s.ward || 'Hope', room_number: s.patient_room || s.current_room || s.room_number || '?',
